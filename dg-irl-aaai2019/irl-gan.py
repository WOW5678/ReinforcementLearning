...
PARSER.add_argument('-vocab_freq', '--vocab_freq', default=0, type=int, help='this is the threshold of word frequency')
...
gen_config.vocab_size=ARGS.vocab_freq ##??确定是正确的
...
def mc_sampler_fast(sess=None, gen_model=None, source_inputs=None, source_outputs=None,
    ...
    for query, answer in zip(source_inputs, source_outputs):
        rep_punish_line = []
        query = query[:query_len] + [int(data_utils.PAD_ID)] * (query_len - len(query) if query_len > len(query) else 0) #数据padding
        train_query.append(query)
        answer.append(data_utils.EOS_ID)
        # it is necessary to regard EOS as a normal action
        last_pos = max(last_pos, len(answer))  #保存最后一个不是pad的位置，方便进行mask
        #对答案进行padding
